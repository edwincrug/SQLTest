CREATE  PROCEDURE [partida].[SEL_PARTIDACONTRATO_DINAMICO_SP] 
	@idTipoObjeto		INT,
...
;with catalogos as(
	select	
		par.idPartidaContrato			idPartidaContrato,
	...
	UNION ALL	
		select
		cat.idPartidaContrato			idPartidaContrato,
		prg.agrupador					agrupador, 
		prg.valor						valor,
		prg.idPadre						idPadre,
		prg.orden						orden,
...
;with tags as(
	...
	UNION ALL	
		select
		tag.idPartidaContrato					idPartidaContrato,
		prg.agrupador					agrupador, 
		prg.valor						valor,
		prg.idPadre						idPadre,
		prg.orden						orden,
	...
	SELECT STUFF(
		...
		(SELECT ', ' + valor
		FROM tags tagsAux
		WHERE tagsAux.idPadre is not null and tagsAux.idPartidaContrato = tags.idPartidaContrato and tagsAux.agrupador = tags.agrupador
		FOR XML PATH ('')),
...
set @query = '
	select
		*
	from
	(select idPartidaContrato as idPartida, agrupador, valor from #partidas) t
	pivot
	(	
		max(valor)
